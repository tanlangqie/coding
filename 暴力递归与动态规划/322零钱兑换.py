...
#                 res.append(path)
#                 return
#             for k,v in enumerate(coins):               #深度优先遍历，有多深是不固定的，但是每一次可选选择的元素是固定的，就是数组coins的个数。
#                 fun(coins[k:],path+[v],amount,res)     #coins[k:]是为了剪枝，   也可以是coins 只不过这样的话就是 每个节点都有len(coins)个子节点，coins[k:]这样是为了剪枝
#             return
