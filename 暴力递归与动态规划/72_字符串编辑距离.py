...
# Author: tangzhuang
# Date:   2024/3/31
# desc: https://leetcode.cn/problems/edit-distance/description/
"""
...
知道"abcd"变成"fgh"多少步（假设X步），那么从"abcde"到"fgh"就可以走的路是"abcd"->"abcde"->"fgh"。（原来的x步，加一次添加，总共X+1步）
知道"abcde"变成“fg”多少步（假设Y步），那么从"abcde"到"fgh"就可以走的路是"abcde"->"fg"->"fgh"。（先Y步，再一次添加，总共Y+1步）
知道"abcd"变成“fg”多少步（假设Z步），那么从"abcde"到"fgh"就可以走的路是"abcde"->"fge"->"fgh"。（先不管最后一个字符，把前面的先变好，用了Z步，然后把最后一个字符给替换了。这里如果最后一个字符碰巧就一样，那就不用替换，省了一步）
...
#定义dp[i][j]的含义为：word1的前i个字符和word2的前j个字符的编辑距离。  第[i][j]个位置只和左边，上面，左上角3个相关
